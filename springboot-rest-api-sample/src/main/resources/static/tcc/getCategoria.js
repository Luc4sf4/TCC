	...
	$.ajax({
		...
		success: function(response) {
			...
			for (let i = 0; i < response.length; i++) {
    			...
    			+response[i].description+'</b></p> <button class="w3-button w3-black w3-margin-bottom" onclick="document.getElementById('+response[i].idCategoria+').style.display="block"">Confira Já!</button></div></div>');
				}
